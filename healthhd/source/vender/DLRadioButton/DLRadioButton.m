...
- (void)sizeToFit {
    ...
    CGFloat marginWidth = self.marginWidth ? self.marginWidth : kDefaultMarginWidth;
    CGFloat iconSize = self.iconSize ? self.iconSize : kDefaulIconSize;
    CGSize tempSize = CGSizeMake(self.frame.size.width -iconSize - 2*marginWidth, 1000);
    CGSize stringSize = [self.titleLabel.text sizeForFont:self.titleLabel.font size:tempSize mode:NSLineBreakByCharWrapping];
    CGFloat cx = MIN(tempSize.width, stringSize.width);
    cx += iconSize + 2*marginWidth;
    self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, cx, MAX(stringSize.height, iconSize + 2*marginWidth));
}
