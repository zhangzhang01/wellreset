...
- (UIView *)customViewForEmptyDataSet:(UIScrollView *)scrollView
{
    if (self.loading) {
        ...
        activityView.tintColor = [UIColor wr_themeColor];
        [activityView startAnimating];
         */
        
        CGFloat size = WRActivityIndicatorViewSize;
        DGActivityIndicatorView *activityView = [[DGActivityIndicatorView alloc] initWithType:DGActivityIndicatorAnimationTypeTriplePulse tintColor:[UIColor wr_themeColor] size:size];
        [activityView startAnimating];
        return activityView;
    }else {
        return nil;
...
- (UIImage *)imageForEmptyDataSet:(UIScrollView *)scrollView
{
    if (self.loading) {
        ...
        NSString *imageName = @"well_default";
...
- (NSAttributedString *)descriptionForEmptyDataSet:(UIScrollView *)scrollView
{
    if (self.loading) {
        ...
        NSString *text = NSLocalizedString(@"没有数据！您可以尝试重新获取", nil);
        ...
        NSDictionary *attributes = @{
                                     NSFontAttributeName: [UIFont systemFontOfSize:([UIFont labelFontSize] + 2)],
                                                    NSForegroundColorAttributeName:[UIColor lightGrayColor],
                                     NSParagraphStyleAttributeName: paragraph
                                                           };
...
- (NSAttributedString *)buttonTitleForEmptyDataSet:(UIScrollView *)scrollView forState:(UIControlState)state
{
    if (self.loading) {
        ...
        return [[NSAttributedString alloc] initWithString:self.buttonText ? self.buttonText : NSLocalizedString(@"重新加载", nil) attributes:attributes];
    }
}
