...
- (void)__flipViewAccordingToStatusBarOrientation {
	...
	if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")) {
		...
		y = floor((screenSize.height - self.bounds.size.height) / 2.0f);//screenSize.height - self.bounds.size.height - 15.0f - TABBAR_OFFSET;
	}
	else {
		UIInterfaceOrientation orientation = [[UIApplication sharedApplication] statusBarOrientation];
		CGFloat angle = 0.0;
...
+ (void)showWithText:(NSString *)text {
	[WToast showWithText:text duration:kWTShort roundedCorners:YES];
}
