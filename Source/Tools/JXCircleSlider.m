...
- (void)drawRect:(CGRect)rect {
    ...
    [[UIColor wr_themeColor] setStroke];
    CGContextSetLineWidth(context, _lineWidth);
...
//    CGContextDrawPath(context, kCGPathStroke);
    ...
    UIImage* im = [UIImage imageNamed:@"数值按钮"];
    CGImageRef image = im.CGImage;
    CGContextSaveGState(context);
    CGRect touchRect = CGRectMake(handleCenter.x-im.size.width*1.0/2, handleCenter.y-im.size.width*1.0/2, im.size.width, im.size.height);
    CGContextSetShadowWithColor(context, CGSizeMake(0, 0), 3,[UIColor wr_themeColor].CGColor);
