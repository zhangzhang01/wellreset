...
-(instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.showsHorizontalScrollIndicator = NO;
        _labelArray = [NSMutableArray array];
        _focusTextColor = [UIColor whiteColor];
        _underLineColor = _focusTextColor;
...
- (void)sizeToFit
{
    CGFloat margin = 0, width = 0;
    if (_labelArray.count > 0) {
        width += margin;
    }
    for (UILabel *label in _labelArray) {
        width += label.width + margin;
    }
    self.frame = [Utility resizeRect:self.frame cx:width height:-1];
}
...
//    CGFloat labelWidth = (self.frame.size.width - (labelCount - 1) * margin) / labelCount;
//    if (labelCount <= 3) {
...
- (void)syncWithScrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    [self scrollToIndex:index needUpdate:YES];
}
...
-(void)scrollToIndex:(NSInteger)index needUpdate:(BOOL)needUpdate
{
    if (needUpdate) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            titleLable.textColor = weakSelf.focusTextColor;
        }];
    } else {
...
-(IBAction)labelClick:(UITapGestureRecognizer*)recognizer
{
    ...
    [self scrollToIndex:label.tag needUpdate:NO];
}
@end
