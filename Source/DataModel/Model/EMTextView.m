...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        _contentColor = [UIColor wr_titleTextColor];
        _placeholderColor = [UIColor wr_detailTextColor];
        _editing = NO;
...
- (void)startEditing:(NSNotification *)notification
{
    _editing = YES;
    NSLog(@"%@",super.text);
    UIColor* cl = super.textColor;
    if ([super.text isEqualToString:_placeholder] && [[super.textColor hexString] isEqualToString: [_placeholderColor hexString]]) {
        super.textColor = _contentColor;
        super.text = @"";
    }
}
