...
- (UIView *)singleWeekView:(ASSingleWeekView *)singleWeekView viewForDate:(NSDate *)date withFrame:(CGRect)frame
{
  ...
  letterLabel.text = [letterLabel.text stringByReplacingOccurrencesOfString:@"周" withString:@""];
  ...
  [wrapper addSubview:numberLabel];
    if ([ [self.dayNumberDateFormatter stringFromDate:date] isEqualToString:[self.dayNumberDateFormatter stringFromDate:[NSDate date]]]) {
        letterLabel.text = @"今";
  ...
  if ([self.delegate respondsToSelector:@selector(weekSelector:circleColorForDate:)]) {
    ...
    if (color) {
      ...
      view.circleColor = color;
        if ([ [self.dayNumberDateFormatter stringFromDate:date] isEqualToString:[self.dayNumberDateFormatter stringFromDate:[NSDate date]]]) {
            view.circleColor = letterLabel.textColor;
        }
