...
-(void) drawValue:(CGContextRef)ctx {
    NSDictionary *attributes = @{ NSFontAttributeName: _labelFont,
                                  NSForegroundColorAttributeName: _labelColor};
    NSString* label = [NSString stringWithFormat:@"%d", (int)self.currentValue];
    CGRect labelLocation = CGRectMake(0, 0, [self widthOfString:label withFont:_labelFont], [self heightOfString:label withFont:_labelFont]);
    
    CGPoint centerPoint = CGPointMake(self.frame.size.width/2, self.frame.size.height/2);
    labelLocation.origin.x =  centerPoint.x - labelLocation.size.width/2;
    labelLocation.origin.y = centerPoint.y - labelLocation.size.height/2;
    [label drawInRect:labelLocation withAttributes:attributes];
}
...
-(void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event{
    ...
    if(_snapToLabels && labelsEvenSpacing != nil) {
        ...
        for (int i=0; i<[labelsEvenSpacing count]; i++) {
            CGFloat percentageAlongCircle = i/(float)[labelsEvenSpacing count];
            CGFloat degreesForLabel = percentageAlongCircle * 360;
            if(fabs((float)(fixedAngle - degreesForLabel)) < minDist) {
