...
@implementation SDPhotoBrowser 
{
    ...
    UILabel *_indexLabel, *_detailLabel;
...
- (void)setupToolbars
{
    ...
    label.frame = CGRectMake(0, 0, self.bounds.size.width - 2*5, 60);
    _detailLabel = label;
    ...
    [saveButton setImage:[UIImage imageNamed:@"well_icon_download"] forState:UIControlStateNormal];
...
- (void)setupDetailOfImageViewForIndex:(NSInteger)index
{
    NSString *text = @"";
    if ([self.delegate respondsToSelector:@selector(photoBrowser:detailForIndex:)]) {
        text = [self.delegate photoBrowser:self detailForIndex:index];
    }
...
//        sourceView = self.sourceImagesContainerView.subviews[currentIndex];
        sourceView = self.sourceImagesContainerView.subviews[currentIndex % self.sourceImagesContainerView.subviews.count];
    }
...
- (void)layoutSubviews
{
    ...
    _saveButton.frame = CGRectMake(self.bounds.size.width - _saveButton.bounds.size.width - 20, self.bounds.size.height - _saveButton.bounds.size.height - 20, _saveButton.bounds.size.width, _saveButton.bounds.size.height);
    
    CGFloat offset = 5;
    [_detailLabel sizeToFit];
    CGFloat cy = MAX(_detailLabel.frame.size.height, 80);
    _detailLabel.frame = CGRectMake(offset, CGRectGetMaxY(_saveButton.frame) - cy, rect.size.width - 2*offset, cy);
}
...
//        sourceView = self.sourceImagesContainerView.subviews[self.currentImageIndex];
         sourceView = self.sourceImagesContainerView.subviews[self.currentImageIndex % self.sourceImagesContainerView.subviews.count];
    }
    ...
    [self setupDetailOfImageViewForIndex:self.currentImageIndex];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (!_willDisappear) {
        ...
        [self setupDetailOfImageViewForIndex:index];
    }
