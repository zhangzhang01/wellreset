...
- (instancetype _Nullable)initWithModel:(ZXSegmentHeaderModel* _Nonnull)model
    ...
    //item宽度：
    flowLayout.itemSize = CGSizeMake(containerView.frame.size.width / 4, self.itemHeight);
...
- (void)changeRotate:(NSNotification*)noti {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        //item宽度：
        flowLayout.itemSize = CGSizeMake(self.containerView.frame.size.width / 6, self.itemHeight);
    });
}
