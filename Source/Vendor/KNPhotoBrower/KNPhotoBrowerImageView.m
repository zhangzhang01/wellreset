...
- (void)sd_ImageWithUrl:(NSURL *)url placeHolder:(UIImage *)placeHolder{
    ...
    [[mgr imageCache] queryDiskCacheForKey:[url absoluteString] done:^(UIImage *image, SDImageCacheType cacheType) {
        ...
        if (image) { // 如果缓存中有图片, 则直接赋值
            ...
            // SDWebImage 下载图片
            [_imageView
                   sd_setImageWithPreviousCachedImageWithURL:url andPlaceholderImage:placeHolder options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
