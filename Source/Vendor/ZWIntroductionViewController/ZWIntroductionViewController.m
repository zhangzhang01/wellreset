...
//        [self.enterButton setTitle:NSLocalizedString(@"Get WELL Today", nil) forState:UIControlStateNormal];
        [self.enterButton setTitleColor:[UIColor wr_themeColor] forState:UIControlStateNormal];
//        self.enterButton.layer.borderWidth = 0.5;
//        self.enterButton.layer.borderColor = [UIColor wr_themeColor].CGColor;
    }
...
- (void)addBackgroundViews
{
    ...
    [[[self.backgroundImageNames reverseObjectEnumerator] allObjects] enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        imageView.tag = idx + 1;
        if (imageView.tag == self.backgroundImageNames.count ) {
...
- (CGRect)frameOfEnterButton
{
    ...
    if (CGSizeEqualToSize(size, CGSizeZero)) {
        size = CGSizeMake(self.view.frame.size.width * 0.6, 220);
    }
...
- (NSArray*)scrollViewPages
{
    ...
    NSMutableArray *tmpArray = [NSMutableArray new];
    [self.backgroundImageNames enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
