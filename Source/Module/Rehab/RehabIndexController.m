...
//                item.detailLabel.text = detail;
            ...
            if (index == 0) {
                ...
                for(NSInteger index = 0; index < MIN(maxCount, array.count); index++)
                {
                    ...
                    if (disease.state == DiseaseStateComming && biPad) {
                        break;
                    }
        ...
        for (NSUInteger index = 0; index < 1; index++)
        {
            ...
            for(WRRehabDisease *obj in self.viewModel.treatDiseaseArray)
            {
            ...
            header = [RCAppUtils createSectionHeaderWithTitle:title icon:iconName tintColor:(index > 0) ? [UIColor wr_themeColor] : [UIColor orangeColor] width:cx more:(index == 0) && (!biPad) moreAction:^{
                if ([weakSelf checkUserLogState:nil]) {
                    [weakSelf showDiseaseSelector];
                }
            }];
            ...
            for(NSString *code in codeSets)
            {
                ...
                if (array.count > 0) {
                    ...
                    for(WRRehabDisease *disease in array)
                    {
                        ...
                        item.tag = [self.viewModel.treatDiseaseArray indexOfObject:disease];
                        if (itemHeight == 0)
                        {
                            [item sizeToFit];
                            itemHeight = item.height;
                        ...
                        [item bk_whenTapped:^{
                            if([weakSelf checkUserLogState:nil]) {
                                NSInteger position = item.tag;
                                WRRehabDisease *disease = self.viewModel.treatDiseaseArray[position];
                                [weakSelf showTreatmentWithDisease:disease];
                            }
            ...
            if (index == 0) {
                ...
                for(NSInteger index = 0; index < MIN(maxCount, array.count); index++)
                {
                    ...
                    if ((index + 1)%columns == 0) {
                        ...
                        x += cx ;
                        if ((index%columns != (columns - 1))) {
                            UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(x, y, lineSize, cy)];
                            lineView.backgroundColor = lineColor;
                            [panel addSubview:lineView];
                        }
                        x += offset;
...
{
    [[self.class root] presentProTreatRehabWithDisease:disease stage:0 upgrade:@"0"];
}
