...
//    textField.returnKeyType = UIReturnKeyNext;
    ...
    messageiconImage.image = [UIImage imageNamed:@"验证码"];
...
//    [textField becomeFirstResponder];
    ...
    textField.frame = CGRectMake(x+5+20, y, cx - 3 * offset - cx0 -10-20, cy);
    y = textField.bottom;
    lineView = [UIView new];
    lineView.backgroundColor = [UIColor colorWithHexString:@"f1f1f1"];
    lineView.frame = CGRectMake(x +10 , y, cx-20 , 1);
...
-(BOOL)canSubmit {
    ...
    flag &= self.codeTextField.text.length ==4;
    return flag;
}
...
- (IBAction)onClickedOKButton:(UIButton *)sender
{
    if (![Utility ValidateMobile:[self pureTextFieldWithString:self.accountTextField.text]]) {
        ...
        [WRViewModel loginWithAccount:[self pureTextFieldWithString:self.accountTextField.text] password:self.codeTextField.text type:@"smsCode" completion:^(NSError * _Nonnull error) {
            if (error) {
  ...
  //  [self.codeTextField becomeFirstResponder];
    ...
    if (pureText.length>0) {
        ...
        if (_sumSeconds < countDownSeconds && _sumSeconds > 0) {
            ...
            ZYEALView* code = [[ZYEALView alloc]initwithPhone:pureText type:@"login"];
            ...
            code.completionBlock = ^(NSString *text, NSString *codeid) {
                ...
                NSLog(@"%@",uptext);
                NSString* md5code = [Utility MD5Encrypt:uptext];
                NSLog(@"%@",md5code);
...
- (IBAction)onClickedSendOnceMore:(UIButton *)sender
{
    ...
    __weak __typeof(self)weakself = self;
        [WRViewModel requestSmsCodeWithPhone:[self pureTextFieldWithString:[self pureTextFieldWithString:self.accountTextField.text]] type:@"login" uuid:codeid code:md5code completion:^(NSError *error) {
        if (error) {
            [SVProgressHUD showErrorWithStatus:NSLocalizedString(@"获取验证码失败", nil)];
        } else {
            [al dismissWithCompletion:^{
