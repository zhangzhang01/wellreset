...
//            [imageV yy_setImageWithURL:[NSURL URLWithString:pic_urls[i]] options:YYWebImageOptionShowNetworkActivity];
            ...
            [imageV sd_setImageWithURL:[NSURL URLWithString:pic_urls[i]] placeholderImage:[UIImage imageNamed:@"well_default"]];
            imageV.hidden = NO;
        }else{
            imageV.hidden = YES;
        }
...
-(NSData*)useImage:(UIImage*)image {
    ...
        CGFloat hfactor = image.size.width*1.0 / [UIScreen mainScreen].bounds.size.width;
    
        CGFloat vfactor = image.size.height*1.0 / [UIScreen mainScreen].bounds.size.width;
