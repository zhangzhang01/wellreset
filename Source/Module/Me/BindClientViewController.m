...
//    bar.tintColor = bar.barTintColor;p
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return indexPath.section == 0 ? image.size.height + 2*WRUIOffset : 52;
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0)
    {
        ...
        else
        {
            ...
            UMSocialPlatformType  platformType = (index == 1)? UMSocialPlatformType_WechatSession :UMSocialPlatformType_QQ;
    ...
    else
    {
        ...
        if ([[WRUserInfo selfInfo] isLogged]) {
            [Utility alertWithViewController:self.navigationController title:NSLocalizedString(@"是否要真的退出当前帐号", nil) buttonText:NSLocalizedString(@"退出", nil) completion:^{
                 [DEFAULTS removeObjectForKey:@"finderHomeVCcity"];
                ...
                [UMessage removeTag:[WRUserInfo selfInfo].userId response:^(id  _Nonnull responseObject, NSInteger remain, NSError * _Nonnull error) {
...
{
//    __weak __typeof(self) weakSelf = self;
//    UIAlertController *controller = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"请输入需要绑定的手机号码", nil) message:NSLocalizedString(@"", nil) preferredStyle:UIAlertControllerStyleAlert];
...
//    [self presentViewController:controller animated:YES completion:nil];
    [self.navigationController pushViewController:[SmsBindViewController new] animated:YES];
    
}

-(void)fetchSmsCode:(NSString*)phone
{
    ZYEALView* code = [[ZYEALView alloc]initwithPhone:phone type:@"bind"];
    ...
    code.completionBlock = ^(NSString *text, NSString *codeid) {
        ...
        [WRViewModel requestSmsCodeWithPhone:phone type:@"bind" uuid:codeid code:md5code completion:^(NSError * _Nonnull error) {
        [SVProgressHUD dismiss];
        if (error) {
